"""
Command-line interface for tidemark.
"""
...
def find_native_binary() -> str:
    ...
    target_binary = project_root / "target" / "release" / "tidemark"
    if target_binary.exists() and not target_binary.is_dir():
        return str(target_binary)

    if sys.platform == "win32":
        target_binary = project_root / "target" / "release" / "tidemark.exe"
        if target_binary.exists() and not target_binary.is_dir():
            return str(target_binary)

    raise FileNotFoundError(
        "Could not find the native tidemark binary. "
        "Please ensure it was built with 'cargo build --release'."
    )


def main() -> int:
    """Run the tidemark command line tool."""
    try:
